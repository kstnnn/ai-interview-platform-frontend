  ...
  <div class="flex min-h-screen flex-col bg-background">
    ...
    <main class="flex flex-1 items-center px-4 py-12 sm:px-6 lg:px-8">
      <div class="mx-auto grid w-full max-w-6xl gap-8 lg:grid-cols-[1fr_0.8fr] lg:items-center">
        <section>
          <p class="text-sm font-semibold uppercase tracking-[0.2em] text-primary">{{ t('signIn.secureAccess') }}</p>
          <h1 class="mt-4 font-serif text-5xl font-bold tracking-tight text-foreground sm:text-6xl">{{ t('signIn.title') }}</h1>
        ...
        <BaseCard class="p-8">
          <h2 class="text-2xl font-bold text-foreground">{{ t('signIn.cardTitle') }}</h2>
          ...
          <div class="mt-8 space-y-3">
            ...
            <RouterLink to="/sign-up" class="block">
              <BaseButton class="w-full" size="lg" variant="outline">{{ t('signIn.createAccount') }}</BaseButton>
            </RouterLink>
          </div>
        </BaseCard>
      </div>
